...
### Пример
Представь, что ты играешь в коробку с секретом.

Коробка (функция): У тебя есть коробка, в которую ты кладешь какую-то вещь. Эта коробка — это функция.

Секрет внутри (переменная): Ты кладешь в коробку секрет (например, конфету) и закрываешь крышку. Этот секрет — это переменная, которая хранится внутри функции.

Ключ к коробке (вложенная функция): Ты даешь другу ключ от коробки. Этот ключ — это вложенная функция, которая может открыть коробку и взять секрет (конфету).

Закрытая коробка с секретом: Даже если ты ушел играть в другое место, друг с ключом все равно может открыть коробку и посмотреть, что внутри. Это и есть замыкание (closure): секрет остается внутри коробки, и только тот, у кого есть ключ, может его использовать.
...
const addLy = Suf("ly");   // 2. В коробку положили секрет "ly"
console.log(addLy("hopeless")); // Друг с ключом использует секрет, добавляя "ly"
console.log(addLy("total"));    // Друг снова использует секрет
```
- Объяснение:
Когда ты вызываешь Suf("ly"), ты создаешь коробку и кладешь туда секрет "ly".
Ты отдаешь ключ (функцию) addLy, который помнит, что в коробке лежит "ly".
Теперь, когда ты вызываешь addLy("hopeless"), ключ берет слово ("hopeless") и добавляет к нему секрет "ly".
Просто:

Замыкание — это "память" функции о том, что лежало в коробке, даже если ты уже ушел и сам не видишь коробку.
